...
@section('subcontent')
    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12 lg:col-span-6">
            <form class="validate-form-teacher" action="{{ route('manager-angi-edit', $angi->id) }}" method="post" enctype="multipart/form-data">
                @csrf
                ...
                <div class="intro-y box p-5">
                    <div class="input-form">
                        ...
                        <input type="text" name="ner" value="{{ $angi->ner }}" class="input w-full border mt-2" minlength="2" 
                            required data-pristine-minlength-message="2 тэмдэгдээс дээш байх ёстой" 
                            data-pristine-required-message="Ангийн нэр хоосон байж болохгүй" />
                    </div>
                                    ...
                                    <optgroup label="{{ $bolovsrol->ner }}">
                                        @foreach($mergejils as $mergejil):
                                            @if($bolovsrol->id == $mergejil->bolovsrol)
                                                @if ($mergejil->bolovsrol == $angi->m_id)
                                                    <option value="{{ $mergejil->id }}" selected> --- {{ $mergejil->ner }}  /{{ $mergejil->jil }} жил/ </option>
                                                @else 
                            ...
                            <select name="b_id" data-search="true" class="tail-select w-full">
                                @if(count($teachers))
                                    ...
                                    @foreach($teachers as $teacher):
                                        @if ($teacher->id == $angi->b_id)
                                            <option value="{{ $teacher->id }}" selected>{{ Str::substr($teacher->ovog, 0, 1) }}. {{ $teacher->ner }}</option>
                                        @else 
                    ...
                    <div class="input-form mt-3">
                        ...
                        <div class="sm:grid grid-cols-3 gap-2">
                            ...
                            <div class="relative mt-2">
                                ...
                                <div class="pl-8">
                                    <input type="text" name="buleg" value="{{ $angi->buleg }}" class="input pl-20 w-full border col-span-4" minlength="1" maxlength="1" required data-pristine-required-message="Бүлэг хоосон байж болохгүй">
                                </div>
                            </div>
                        </div>
                    </div>
